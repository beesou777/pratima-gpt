<script lang="ts">
    ...
    const handleScroll = () => {
        ...
        isActive.set(currentScrollTop >= threshold);
    };
    ...
    const routerLinkActive = (link: string) => {
...
>
    ...
    >
        ...
        <div class="lg:hidden order-3 flex justify-end col-span-1">
            ...
            >
                <span
                    class={`w-full h-[2px] origin-[0%_50%] [transition:0.2s_cubic-bezier(0.34,_1.56,_0.64,_1)] rounded-full bg-gray-900 ${$isOpen ? "rotate-45" : "rotate-0"}`}
                ></span>
                <span
                    class={`w-[75%] h-[2px] origin-[0%_50%] [transition:0.2s_cubic-bezier(0.34,_1.56,_0.64,_1)] rounded-full bg-gray-900 ${$isOpen ? "opacity-0" : "opacity-100"}`}
                ></span>
                <span
                    class={`w-[50%] h-[2px] origin-[0%_50%] [transition:0.2s_cubic-bezier(0.34,_1.56,_0.64,_1)] rounded-full bg-gray-900 ${$isOpen ? "-rotate-45 !w-full" : "rotate-0"}`}
                ></span>
            </button>
        </div>
        <div
            class={`backdrop-filter rounded-[.75rem] order-2 col-span-2 menubar ${$isOpen ? "menubar--open" : "menubar--closed"} ${$isActive ? "" : "backdrop-blur-[5px] bg-[rgba(42,42,42,0.05)]"}`}
        >
            <nav
