...
<main
  class="max-w-[1450px] mx-auto w-full bg-secondary {$chatHistry.length > 0
    ? 'mb-[95px]'
    : ''}"
>
    ...
    {#if Array.isArray($chatHistry) && $chatHistry.length > 0}
      {#each $chatHistry as value, index (index)}
        <div class="md:max-w-[40%] md:ml-auto max-w-full w-full">
          ...
          <div class="user-response bg-gray-950 text-white px-4 rounded my-2">
            <p>{value.prompt}</p>
          </div>
        </div>
        <div class="md:max-w-[45%] max-w-full w-full">
          ...
          {#if !value.response}
            <div class="bg-gray-950 px-4 rounded my-2">
